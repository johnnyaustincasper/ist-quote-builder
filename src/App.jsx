...
var LOCATIONS = [
  { id: "ext_walls_house", label: "Boxed exterior walls of house", type: "wall", group: "Exterior Walls" },
  { id: "ext_walls_garage", label: "Boxed exterior walls of garage", type: "wall", group: "Exterior Walls" },
  { id: "ext_slopes", label: "Boxed exterior slopes", type: "wall", group: "Exterior Walls" },
  { id: "garage_common", label: "Garage common wall", type: "wall", group: "Exterior Walls" },
  { id: "ext_kneewall", label: "Boxed exterior kneewall", type: "wall", group: "Exterior Walls" },
  ...
  { id: "gable_end", label: "Gable end", type: "area", group: "Attic / Ceiling" },
...
var GROUP_ORDER = ["Exterior Walls","Attic / Ceiling","Porch / Blocking","Roofline","Other"];
...
function MeasurementForm(p){
  ...
  function handleAdd(){
    ...
    p.onAdd({type:p.tab==="foam"?"Foam":"Fiberglass",material:mat,location:locLabel,locationId:loc?loc.id:"custom",group:locGroup,sqft:fin,pitch:needsPitch?pitch:null,pricePerUnit:pr,total:hp?fin*pr:0,description:"Install "+mat.toLowerCase()+" in "+locLabel.toLowerCase()});
  ...
  return(<div style={{background:C.card,borderRadius:12,padding:16,border:"1px solid "+C.border}}>
    ...
    <div style={{marginBottom:12}}><StepLabel>{"① Location"}</StepLabel><select style={ss} value={lid} onChange={function(e){setLid(e.target.value);setSqft(0);setMk(function(k){return k+1;});}}><option value="">{"— Select location —"}</option>{LOCATIONS.map(function(x){return(<option key={x.id} value={x.id}>{x.label}</option>);})}</select></div>
    ...
    {loc&&(<div>
      ...
      {fin>0&&(<div style={{background:C.bg,borderRadius:8,padding:12,marginBottom:12,fontSize:13,color:C.dim,border:"1px solid "+C.border}}>
        ...
        {hp&&(parseFloat(price)||0)>0&&(<div>{"Line Total: "}<span style={{color:C.green,fontWeight:700}}>{"$"+(fin*(parseFloat(price)||0)).toFixed(2)}</span></div>)}
      </div>)}
...
function QuoteBuilderSection(p){
  ...
  function addItem(item){p.setQuoteItems(function(prev){return prev.concat([Object.assign({},item,{id:Date.now()+Math.random()})]);});}
  function removeItem(id){p.setQuoteItems(function(prev){return prev.filter(function(i){return i.id!==id;});});}
  var unpriced=p.importedItems.filter(function(i){return!i.priced;});
  var subtotal=p.quoteItems.reduce(function(s,i){return s+i.total;},0);
  function handlePriceImport(item){var pr=parseFloat(pricingPrice)||0;if(pr<=0)return;
    addItem(Object.assign({},item,{pricePerUnit:pr,total:item.sqft*pr}));
      ...
      <div style={{background:C.card,borderRadius:10,border:"1px solid "+C.border,overflow:"hidden"}}>
        {unpriced.map(function(item,idx){return(<div key={item.id} style={{padding:"12px 14px",borderBottom:idx<unpriced.length-1?"1px solid "+C.border:"none"}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
            ...
            {pricingId===item.id?(<div style={{display:"flex",gap:6,alignItems:"center",marginLeft:12}}>
            ...
            </div>):(<button onClick={function(){setPricingId(item.id);}} style={{marginLeft:12,padding:"6px 14px",background:"transparent",border:"1px solid "+C.green,borderRadius:8,color:C.green,fontSize:12,fontWeight:700,cursor:"pointer",fontFamily:"'Outfit',sans-serif",textTransform:"uppercase"}}>{"Price"}</button>)}
          </div>
        </div>);})}
      </div>
    </div>)}
    ...
    {p.quoteItems.length>0&&(<div style={{padding:"0 16px 20px"}}>
      ...
      <div style={{background:C.card,borderRadius:12,padding:16,border:"1px solid "+C.border}}>
        {p.quoteItems.map(function(item,idx){return(<div key={item.id} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"10px 0",borderBottom:idx<p.quoteItems.length-1?"1px solid "+C.border:"none"}}>
          ...
          <div style={{textAlign:"right",marginLeft:12}}><div style={{fontSize:15,fontWeight:700,color:C.green}}>{"$"+item.total.toFixed(2)}</div><button onClick={function(){removeItem(item.id);}} style={{background:"none",border:"none",color:C.danger,fontSize:11,cursor:"pointer",fontFamily:"'Outfit',sans-serif",fontWeight:600,marginTop:2}}>{"REMOVE"}</button></div>
        </div>);})}
        <div style={{paddingTop:12,marginTop:8,borderTop:"1px solid "+C.borderLight}}><div style={{display:"flex",justifyContent:"space-between",padding:"12px 0 0",fontSize:18,fontWeight:800,color:C.white}}><span>{"TOTAL"}</span><span>{"$"+subtotal.toFixed(2)}</span></div></div>
      </div>
      <GreenBtn mt={12} onClick={function(){generatePDF({name:p.custName,address:p.custAddr,phone:p.custPhone,email:p.custEmail,jobAddress:p.jobAddr},p.quoteItems,subtotal);}}>{"Generate Quote PDF"}</GreenBtn>
  ...
  return (
    <div style={{ fontFamily: "'Outfit', sans-serif", background: C.bg, color: C.text, minHeight: "100vh", maxWidth: 520, margin: "0 auto", paddingBottom: 100 }}>
      ...
      <div style={{ background: C.card, padding: "22px 20px 16px", borderBottom: "2px solid " + C.green, textAlign: "center", position: "sticky", top: 0, zIndex: 100 }}>
        <h1 style={{ fontSize: 18, fontWeight: 800, color: C.white, letterSpacing: "0.04em", margin: 0, textTransform: "uppercase" }}>{"IST Quote Builder"}</h1>
